...
from .serializer import ProductSerializer, CategorySerializer, CartSerializer, CartItemSerializer, RegisterSerializer, UserSerializer 
...
def add_to_cart(request):
  ...
  try:
    ...
    return Response({"error": "Product isn't available"}, status=404)
  cart, created = Cart.objects.get_or_create(user=request.user)
...
#validate Phone Number 
    if not phone.isdigit() or len(phone) < 10:
      return Response({'error': 'Invalid Phone Number, Please enter a valid phone number'}, status=400)
