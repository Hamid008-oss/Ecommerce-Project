  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMsg(
          data.username || data.password || JSON.stringify(data)
        );
      }
    } catch (error) {
      console.error(error)
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6 bg-[#121212]">
      <div className="max-w-md w-full bg-[#1A1A1A] p-8 rounded-3xl shadow-[0_0_40px_rgba(0,0,0,0.5)] border border-gray-800 relative overflow-hidden">
        ...
        {msg && (
          <p className={`mt-4 text-center text-sm font-medium ${msg.includes('successfully') ? 'text-emerald-400' : 'text-rose-400'}`}>
            {msg}
          </p>
        )}
